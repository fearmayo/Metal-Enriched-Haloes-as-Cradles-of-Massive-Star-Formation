...
stellar mass and the median of each bin again calculated. Haloes are futher subdivied by their
metallicity as shown. In each case we see that the \rarepeak region has a near
systematically higher infall rate relecting the more clustered environment of the \rarepeak
...
should be multiplied by a factor between $10^{-3} - 10^{-4}$ to account for this bias.
Doing this we find a number density of approximately $1 \times 10^{-5}$ cMpc$^{-3}$ as a global estimate.
...
confirms and enfoces the result found in \cite{Wise_2019} that there is likely to be a
