...
\begin{abstract}
  The formation of supermassive stars has generally been studied under the assumptions of rapid accretion of pristine near metal-free gas. Recently it was suggested, however, that gas enriched to metallicities up to $Z \sim 10^{-3}$ \zsolar can also facilitate supermassive star formation, as long as the total mass infall rate onto the protostar remains sufficiently high. We extend the analysis further by examining
  how the abundance of supermassive star candidate haloes would be affected in the case where haloes, with super-critical infall rates, with all levels of metallicity
  ...
  We find that among these haloes with central mass infall rates above 0.1 (1.0) \msolaryrc\ approximately
  ...
  number of haloes hosting supermassive stars can be increased by at least a factor of five.
  Additionally these high infall rate haloes provide ideal environments in which to grow
  pre-existing black holes in the halo centres. Further research into the star
  formation dynamics of high mass infall haloes, with inhomogeneous
  metal distributions, is now required to gain more insight into both supermassive star
...
is currently unknown, and as yet we have no direct observational guidance. However, simulations, as well as the lack of any \change{observed} zero-metallicity stars in our Galaxy, point to an IMF that
...
eight orders of magnitude within a \change[few]{several} hundred million years if they are to grow to become a SMBH. Such
a scenario has been shown to be exceedingly unlikely. Black holes born from PopIII remnants \change[tend to
be born starving]{exist in relic ionized regions that are warm and diffuse, inhospitable for growth \citep{Whalen_2004, Milosavljevic_2009, Alvarez_2009}.}  \change{After these seeds incorporate into galaxies, they} often finding themselves in underdense regions away from the \change{halo} centre \delete{of the halo in
which they live} \citep{Smith_2018}. The black holes
...
Our analysis expands on similar earlier searches for supermassive star candidate haloes (e.g.,~\citealt{Habouzit_2016,Wise_2019,Regan_2020}) into the regime of haloes that are enriched with metals, selecting candidates based on their infall rates, rather than on the requirement of being metal-free.

\note{I feel that we need another name for the ``high mass infall rate haloes'' because it's confusing beginning the phrase with ``high mass'' and it's a mouthful.  I don't have any good suggestions -- ``rapidly collapsing halos''?}

This paper is organised as follows.
In \S~\ref{Sec:RenaissanceDatasets}, we describe the \change[Renaissance Datasets]{datasets that comprise the Renaissanace Simulations} and our analysis methods.
...
The datasets used in this study were originally derived from a simulation of the universe in a \change[(28.4
\mpch)]{(40 Mpc)}$^3$  box using the WMAP7 best fit cosmology~\citep{Komatsu_2011}.
...
regions. \delete{The \void region is not studied here as it is not expected to 
host a significant number of high-infall rate haloes.} \jhw{Repetitive with the next sentence.} In our previous work
\citep{Wise_2019, Regan_2020} \change[the]{we found the} \void region \delete{was found} to be devoid of metal-free and
star-free atomic cooling haloes as infall rates
did not support their formation\change{, thus we do not include it in this study}. 
...
have masses greater than the atomic cooling limit, \change[In this work, we
adopt the calibration of  
 the halo mass corresponding to this 
limit]{which we adopt} from \cite{Fernandez_2014},
%
\begin{equation}
  M_{\rm atm} = 2 \times 10^{7} \left( \frac{{T_{\rm vir}}}{10^4~{\rm K}}  \frac{21}{1+z} \right)^{-1.5} \rm{M_{\odot}},
\end{equation}
%
\change{that is calibrated with simulations,} where $z$ is the redshift and $T_{\rm vir}$ is the virial temperature of the halo. 
...
is very sensitive to the \change[precise value chosen]{chosen value because it lies on the low-mass exponential part of the halo mass function}. We chose a value of the atomic limit consistent
...
\delete{We therefore feel that this is the most appropriate choice for the atomic limit.}\jhw{We've already stated our reasons, and I think this doesn't add anything to the paragraph} 
For both datasets, we then filter each halo based on the mass infall rate \change[onto]{in} that halo.
...
\note{About the averages footnote: Do you mean volume-weighted mean? Because if you specify None as the weight in yt, it will treat all of the cells equally.  This is incorrect for a region average because the differing cell volumes.}
...
\delete{As the plot runs from left to right, the number of haloes with a
mass infall rate greater than a given value diminishes as expected.}\jhw{By definition, a CDF does this and doesn't need to be stated.} The mass infall rate,
$\dot{M}(R)$, is calculated as the average value within 20 pc of the centre for both the \rarepeak 
and \normal regions. \delete{In Figure~\ref{Fig:Histogram}} The
...
haloes most are either EMP or metal-free (59\%). \jhw{I wouldn't say ``most.'' Maybe ``slightly more than half''?} There are very few additional haloes in the
...
mixing within haloes is likely to be somewhat inhomogeneous \change{and incomplete}. \\
...
However, given that these \delete{are relatively massive} \jhw{I wouldn't call them massive since they're similar in mass to low-mass dwarfs.} galaxies have deeper potential wells \change{and longer sound crossing times},
...
measured at the spatial resolution ($\sim 20$pc) achieved here \jhw{Reword? This isn't our resolution limit, but it ensures that the central region is well-sampled.}, 
%
does not guarantee
...
\change[Investigation of]{The topic of} metal inhomogeneity was outside the scope of \delete{the} \cite{Chon_2020} \delete{investigations}
...
very high central \change[accretion]{infall} rates (see Fig.~\ref{Fig:Histogram}). At \change[accretion]{infall} rates
...
reaching yet the high redshifts probed in this study, e.g., \change[WFIRST]{Nancy Grace Roman Space Telescope} and Euclid). Nonetheless, 
