...
\begin{abstract}
  The formation of super-massive stars has generally been studied under the assumptions of rapid
  accretion of pristine near metal-free gas. Recently it was suggested, however, that gas enriched to
  metallicities up to $Z \sim 10^{-3}$ \zsolar can also facilitate super-massive star formation,
  as long as the total mass infall rate onto the proto-star remains sufficiently high. We extend the
  ...
  Renaissance simulations with mass infall rates in their centres exceeding a fixed threshold.
  %between 0.1-1 \msolaryrc. 
  We find that among these
  haloes with central mass infall rates above 0.1 (1.0) \msolaryrc\ approximately
  ...
  number of haloes hosting super-massive stars can be increased by up to an order of magnitude or more.
  Additionally these high in-fall rate haloes provide ideal environments in which to grow
  pre-existing black holes in the halo centres. Further research into the star
  formation dynamics of high mass infall haloes, with in-homogeneous
  metal distributions, is now required to gain more insight into both super-massive star
...
%atomic cooling haloes over all redshift outputs for
  ...
  grayscale, are used to represent the metal poor haloes ($Z > 10^{-3}$ \zsolarc).
  %ZH:  Hm... these were defined as "metal-enriched".. and footnote#2 specifically promised not to call these  "metal-poor".    I suggest to change the above to "metal-enriched", but I am happy with any choice, as long as we are consistent. 
  Darker colours
  signify higher metallicity as well as high halo counts. Blue, red and green circles
  ...
  completely metal-free \underline{and} star-free.  \textit{Right Panel}: The same plot for 
    %ZH: Added sentence below. OK?
  the 2335 halos between $11.6<z<24$ in 
  %
  the \normal simulation
...
classify them.
  ...
  %ZH: I expanded below.  If you think it's too long, it may fit better now in the discussion.
...
%ZH: I added reference to WMAP7.  Is it correct?
...
%ZH: are these Mpc, or h^-1 Mpc like the 28.4 box size given above?
...
%ZH: 1.65 or 1.68 stated above?
volume which is 1.65 times denser than the cosmic mean - therefore at $z = 15$, regions with the
...
%ZH: why 100 times?  The last sentence implies 10^3 to 10^4 times.
rarer than the \normal region).
The \rarepeak regions contains 822 galaxies with masses larger than $2 \times 10^7$ \msolar
at $z = 15$, the \normal region contains 58 such galaxies at $z = 11.6$, while the \void region
...
%ZH: I simplified the caption (and also moved some text, which was not specific to this figure, to Fig.1)
...
\caption{\textit{Left Panel}: The cumulative number of haloes with infall rates greater than the given
  ...
  metal-free haloes. 
  %ZH: Text said factor of 10 or more. Lets be consistent...
 } \label{Fig:Histogram}
\end{center} \end{minipage}
...
%ZH: made this a section:
\section{Results} \label{Sec:Results}

%ZH: And made this into a subsection:
...
%ZH: terminology here differs from definitions in the Introduction:
metal-poor and metal-free haloes. Red circles indicate
mildly metal enriched regions ($10^{-5}$~\zsolar $ \lesssim Z \lesssim 10^{-3}$~\zsolar )
...
metal-poor haloes in the \rarepeak region - the highest mass infall-rate halo is in fact metal-enriched,
...
dynamical and compressional heating. The other metal-poor haloes, especially those with gas temperatures
...
%ZH : added 2335. OK?   But then 84 is more like 3.5%, so I guess either 5% or 2335 is not correct..?
the 2335 haloes
%all of the haloes 
plotted, only 84 ($\sim 5$\%)  have metallicites
...
with stellar masses of M$_{*} \lesssim 10^3$ \msolarc. %ZH: This sounds interesting.  Is this because the PopIII stars were sampled from an IMF and many happened to be in the direct BH collapse range?   Or just too few PopIII stars for Z to rise above 1e-5 ?    Would be worth adding a clarifying comment here.
These are haloes
populated with PopIII stars but the majority are still metal-free ($Z \lesssim 10^{-5}$ \zsolarc).
...
%ZH: I added "e.g." and comment here (key is there extrapolated inflow rate closer to r=0, and I am not sure that it's entirely controlled by metallicity)
...
\ref{Sec:Discussion}. First we quantify the relative abundance of metal-enriched, extremely metal-poor
...
%ZH: I changed below to 20pc. OK?
$10^{-4}$ \msolaryr $\leq \dot{M}(R=20~{\rm pc}) \leq 10^{1}$ \msolaryrc. 
%$\dot{M}(R) = 10^{-4}$ \msolaryr to 
%$\dot{M}(R) = 10^{1}$ \msolaryrc. 
...
shall now see. We will come back to this point in \S \ref{Sec:Discussion} below. \\
...
then the number of possible candidate haloes can increase by more than an order of magnitude.
...
%ZH: can you separately state the EMP and metal-free numbers here, rather than just their sum of 49?  This is to support the statement above about the order-of-magnitude increase.
By comparison, there are 49 extremely metal-poor or metal-free haloes with mass infall rates  greater than 0.1 \msolaryrc. Hence, the vast majority of high mass infall-rate haloes are metal-enriched and could potentially support SMS formation or 
...
%ZH: just to clarify, these are all spherically averaged (in shells with radius r) quantities, yes?
...
%ZH: added:
measured at the spatial resolution ($\sim 10$pc) achieved here, 
%
does not guarantee
...
%ZH:  I rephrased this, since I think most papers claim a sufficient abundance... no??
%Previous SMS candidate halo number densities have tended to show that when including metal-free haloes only that the number of SMS candidate haloes may be either insufficient or just sufficient to explain the number of density of high-z quasars 
...
%ZH:  I moved this down below:
%\cite{Tagawa_2020} came to similar conclusions
%using a semi-analytic approach.
...
result is a SMS. 
%ZH: I moved this down here... since this (fragments coalescing rapidly and keeping the central protostar bloated) is what we predicted in Tagawa et al... not rapid gas accretion.
\cite{Tagawa_2020} came to similar conclusions
...
magnitude. Some of these haloes may ultimately be shown through higher-resolution simulations not to support SMS formation. Nevertheless, their high mass infall rates will, at a minimum, transfer a large amount of 
...
then metal inhomogenitety may be common in the cores of these young protogalaxies. The galaxies studied here are
embryonic and have had very few dynamical times in which 
gas mixing can take place and hence extremely metal-poor pockets are much more likely than in more mature and evolved protogalaxies.\\
...
%ZH: added last sentence not to fully tie ourselves to the rare quasars.
